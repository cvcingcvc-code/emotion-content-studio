...
function mapGeneratedContent(row: GeneratedContentRow): GeneratedContent {
  return GeneratedContentSchema.parse({
    ...
    reviewDecision: row.reviewDecision as "accepted" | "rejected" | null,
    humanEditedOutput: row.humanEditedOutput,
  ...
  async saveGenerated(content: GeneratedContent): Promise<void> {
    ...
    await this.#db
      ...
      .values({
        ...
        promptVersion: content.promptVersion ?? "legacy.v1",
        reviewDecision: content.reviewDecision ?? null,
        humanEditedOutput: content.humanEditedOutput ?? null,
